...
const state = { orders: [], menu: [], unsubscribeOrders: null };
...
function setOrders(orders) {
  state.orders = orders.filter((order) => order.status === 'queued').sort((first, second) => new Date(first.placed_at) - new Date(second.placed_at));
  const allOrders = orders;
  document.getElementById('queued-count').textContent = state.orders.length;
  document.getElementById('preparing-count').textContent = allOrders.filter((order) => order.status === 'preparing').length;
  renderOrders();
}

function renderOrders() {
  const [currentOrder, ...upcomingOrders] = state.orders;
