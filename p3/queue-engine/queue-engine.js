...
export async function createOrder(items) {
  ...
  return withRetry(async () => {
    ...
    const totalItems = itemCount(activeResult.data) + itemCount([{ items }]);
...
export function listenToMenu(callback, { onError = () => {} } = {}) {
  ...
  async function fetchAndEmit() {
    const { data, error } = await supabase.from('menu').select('id, name, price, available').order('name');
...
export async function recalculateWaitTimes() {
  ...
  const updates = activeOrders.map((order) => {
    itemsAhead += itemCount([order]);
    const minutes = Math.ceil(itemsAhead / config.max_concurrent_prep) * config.avg_prep_time_per_item_min;
  ...
  const channel = supabase.channel('orders-changes')
    ...
    .subscribe((status) => {
      ...
      if (status === 'CHANNEL_ERROR' || status === 'TIMED_OUT' || status === 'CLOSED') {
        onConnectionState('disconnected'); scheduleRetry();
      }
    });

  const unsubscribe = () => { stopped = true; clearTimeout(retryTimer); supabase.removeChannel(channel); };
