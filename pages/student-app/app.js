...
const state = { menu: [], cart: new Map(), order: null, unsubscribe: null, queueUnsubscribe: null, menuUnsubscribe: null, api: null, demoMode: true, activeOrders: [], noticeTimer: null };
    ...
    state.api = {
      ...
      listenToOrder: (orderId, onChange) => {
        ...
        const emitLatest = async () => {
          ...
          if (!stopped && !error && data) onChange(data);
        };
        emitLatest();
...
function startMenuListener() {
  ...
  state.menuUnsubscribe = state.api.listenToMenu((items) => {
    state.menu = items.map(normaliseMenu);
...
function showConfirmation(order) {
  ...
  state.unsubscribe?.(); state.unsubscribe = state.api.listenToOrder(state.order.id, (latest) => {
    const wasCancelled = state.order?.status === 'cancelled'; 
    const wasReady = state.order?.status === 'ready';
    state.order = normaliseOrder(latest); 
...
function resetForNewOrder() {
  state.unsubscribe?.(); state.unsubscribe = null; state.order = null; state.cart.clear();
...
await setupApi(); initCart(); await loadMenu(); startMenuListener(); startQueueListener();
